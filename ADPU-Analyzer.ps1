...
function Test-ADPUAuditReady {
    <#
        Reads the two audit subcategories this tool depends on, separately:

          Logon/Logoff  > Logon                          -> event 4624 (NTLM)
          Account Logon > Kerberos Authentication Service -> event 4768 (DES/RC4)

        They are independent settings, so one being on says nothing about the other.
        Both are queried by GUID rather than by name - the names are localised, the
        GUIDs are not. The whole check runs inside the remote session as a
        self-contained block, so nothing has to be shipped to the remote runspace.

        Returns an object with .Logon / .Kerberos (bool) plus the raw settings for
        display. "On" means the subcategory records successes; failure auditing is
        useful but not required, since 4624/4768 are success events.
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)] $Controller)
    ...
    $controllers = foreach ($d in $domains) {
        ...
        foreach ($dc in [DirectoryServices.ActiveDirectory.DomainController]::FindAll($ctx)) {
            ...
            if (-not $dc.WinRMUp) {
                ...
                $dc | Add-Member -NotePropertyName AuditKerbOk  -NotePropertyValue $false -Force
                ...
                $auditLogon = $false; $auditKerb = $false
                try {
                    $audit = Test-ADPUAuditReady -Controller $dc
                    if ($audit) {
                        $auditLogon = [bool]$audit.Logon
                        $auditKerb  = [bool]$audit.Kerberos
                    }
                } catch { }

                $dc | Add-Member -NotePropertyName AuditLogonOk -NotePropertyValue $auditLogon -Force
                $dc | Add-Member -NotePropertyName AuditKerbOk  -NotePropertyValue $auditKerb  -Force
                # AuditOk stays as the "anything at all readable here" summary flag.
                $dc | Add-Member -NotePropertyName AuditOk      -NotePropertyValue ($auditLogon -or $auditKerb) -Force
...
function Show-ADPUReadinessReport {
    ...
    $ntlmGap  = @($Topology.Controllers | Where-Object { -not $_.AuditLogonOk })
    $kerbGap  = @($Topology.Controllers | Where-Object { -not $_.AuditKerbOk })
    ...
    if ($ntlmGap) {
        Write-ADPULine warn ("{0} controller(s) had Logon auditing off - NTLM findings there are missing, not clean (details at the end)." -f $ntlmGap.Count)
    }
    if ($kerbGap) {
        Write-ADPULine warn ("{0} controller(s) had Kerberos Authentication Service auditing off - DES/RC4 findings there are missing, not clean." -f $kerbGap.Count)
    }
    & $wait
    ...
    foreach ($d in $Topology.Domains) {
        ...
        foreach ($dc in $dcs) {
            ...
            $kerbText  = if ($dc.AuditKerbOk)  { 'Kerberos auditing on' }     else { 'Kerberos auditing OFF (no DES/RC4 evidence)' }
            $auditText = "$logonText, $kerbText"
            $fullAudit = $dc.AuditLogonOk -and $dc.AuditKerbOk

            if ($dc.OSOk -and $fullAudit) {
                ...
                if (Read-ADPUYesNo "   Show the command(s) to turn the missing auditing on for $($dc.Name)?") {
                    if (-not $dc.AuditLogonOk) {
                        Write-ADPULine snippet "Invoke-Command -ComputerName $($dc.Name) -ScriptBlock { auditpol /set /subcategory:{0CCE9215-69AE-11D9-BED3-505054503030} /success:enable /failure:enable }"
                    }
                    if (-not $dc.AuditKerbOk) {
                        Write-ADPULine snippet "Invoke-Command -ComputerName $($dc.Name) -ScriptBlock { auditpol /set /subcategory:{0CCE9242-69AE-11D9-BED3-505054503030} /success:enable }"
                    }
                    Write-ADPULine tip 'Subcategories are addressed by GUID so the command works on any system language. Run it with rights over that controller.'
...
'@
    ...
    if ($ntlmGap.Count) {
        & $add ('<section><p class="banner warn">{0} controller(s) had Logon auditing off - NTLM findings there are missing, not clean.</p></section>' -f $ntlmGap.Count)
    }
    if ($kerbGap.Count) {
        & $add ('<section><p class="banner warn">{0} controller(s) had Kerberos Authentication Service auditing off - DES/RC4 findings there are missing, not clean.</p></section>' -f $kerbGap.Count)
    }
    ...
    foreach ($d in $Topology.Domains) {
        ...
        foreach ($dc in $dcs) {
            ...
            $lgCls = if ($dc.AuditLogonOk) { 'ok' } else { 'bad' }
            $lgTxt = if ($dc.AuditLogonOk) { 'on' } else { 'off' }
            $kbCls = if ($dc.AuditKerbOk)  { 'ok' } else { 'bad' }
            $kbTxt = if ($dc.AuditKerbOk)  { 'on' } else { 'off' }
            & $add ('<tr><td>{0}</td><td class="{1}">{2}</td><td class="{3}">{4}</td><td class="{5}">{6}</td></tr>' -f (enc $dc.Name), $osCls, (enc $dc.OSVersion), $lgCls, $lgTxt, $kbCls, $kbTxt)
        }
        & $add '</tbody></table>'
    }
    & $add '</section></main>'
...
function Get-ADPUClearReasons {
    ...
    # Deliberately separate: a controller that audited logons but not Kerberos can
    # honestly back the NTLM line and must not back the DES/RC4 one.
    $ntlmSeen = @($Topology.Controllers | Where-Object { $_.AuditLogonOk })
    $kerbSeen = @($Topology.Controllers | Where-Object { $_.AuditKerbOk })
    $shorten  = {
        param($list)
        $names = @($list.Name | Sort-Object)
    ...
    if ($blind.Count -and ($ntlmSeen.Count -or $kerbSeen.Count)) {
        & $say 'warn' ('{0} controller(s) were missing at least one of the two audit subcategories - anything that happened only there is invisible to this verdict' -f $blind.Count)
    }
